...
import HospitalSidbar from '@/modules/hospital/components/HospitalSidbar';
...
const EditHospital = () => {
  ...
  const hospital = location.state?.hospital as IHospital;
  ...
  const loadHospitalData = (data: IHospital) => {
    ...
    setFormData({
      ...
      email: data.email || '',
      phone: data.phone || '',
      since: Number(data.since) || new Date().getFullYear(),
...
>>>>>>> c8a5339 (fix: final removal of secrets and hospital edit logic)
    try {
      ...
      if (!targetId) {
        toast.error("Hospital ID not found. Please try again from the list.");
        return;
      }
      ...
      Object.entries(formData).forEach(([key, value]) => {
        if (key !== 'logo' && key !== 'licence' && key !== 'subscription') {
      ...
      if (licenseFile) {
        submissionData.append('licence', licenseFile);
      }

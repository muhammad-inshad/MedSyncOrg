...
const SuperAdminWallet = () => {
  ...
  const [transactions, setTransactions] = useState<
    { amount: number; type: 'credit' | 'debit'; date: string }[]
  >([]);
  ...
  const fetchWalletData = async () => {
    try {
      ...
      const result = await superAdminApi.getwallet();
  ...
  setTransactions([...walletData.transactions].reverse()); // newest first
} else {
          setTransactions([]);
        }
      } else {
  ...
  const handleWithdraw = async () => {
    ...
    if (amt > balance) {
      return setWithdrawError(
        `Amount exceeds available balance of ₹${balance.toLocaleString('en-IN')}.`
      );
    }

    const result = await superAdminApi.withdraw({ amount: amt });
        ...
        <div className="grid grid-cols-3 gap-4 max-w-6xl">
          ...
          <div className="col-span-2 space-y-4">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
              ...
              {success && (
                <p className="text-xs text-green-600 text-center font-medium">{success}</p>
              )}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm text-gray-700">
                            {t.type === 'credit' ? 'Booking Payment' : 'Withdrawal'}
                          </p>

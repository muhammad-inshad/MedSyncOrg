...
const MOCK_DEPARTMENTS: IDepartment[] = [
  { _id: 'd1', departmentName: 'Cardiology' },
  { _id: 'd2', departmentName: 'Neurology' },
  { _id: 'd3', departmentName: 'Orthopedics' },
];

const MOCK_SPECIALIZATIONS: ISpecialization[] = [
  { _id: '1', department_id: 'd1', name: 'Interventional Cardiology', description: 'Catheter-based treatment of heart diseases.', image: '', isActive: true },
  { _id: '2', department_id: 'd1', name: 'Electrophysiology', description: 'Diagnosis of heart rhythm disorders.', image: '', isActive: true },
  { _id: '3', department_id: 'd2', name: 'Stroke Medicine', description: 'Acute and long-term management of stroke.', image: '', isActive: false },
  { _id: '4', department_id: 'd3', name: 'Sports Medicine', description: 'Treatment of sports-related injuries.', image: '', isActive: true },
];

const EMPTY_FORM: SpecializationForm = { name: '', description: '', department_id: '', image: '' };
...
const ModalForm = ({
  ...
  onChange: (field: keyof SpecializationForm, value: string) => void;
      ...
      <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            className={`w-full px-4 py-3 bg-slate-50 border rounded-xl text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:bg-white transition-all appearance-none cursor-pointer ${
          ...
          {errors.department_id && <p className="text-xs font-bold text-rose-500">{errors.department_id}</p>}
        </div>
      ...
      <div className="flex items-center justify-between px-8 py-5 border-t border-slate-100 bg-slate-50/50">
        ...
        >
          {isSubmitting ? (
            ...
            <><Stethoscope className="w-4 h-4" /> {submitLabel}</>
          )}
        </button>
      </div>
    </div>
...
const SpecializationManagement = () => {
  const [specializations, setSpecializations] = useState<ISpecialization[]>(MOCK_SPECIALIZATIONS);
  ...
  const getDepartmentName = (id: string) =>
    MOCK_DEPARTMENTS.find((d) => d._id === id)?.departmentName || '—';

  const handleToggleStatus = (id: string) => {
    setSpecializations((prev) => prev.map((s) => s._id === id ? { ...s, isActive: !s.isActive } : s));
  };
  ...
  const handleAddSubmit = async () => {
    ...
    if (!addForm.department_id) errors.department_id = 'Please select a department.';
  ...
  const openEdit = (spec: ISpecialization) => {
    setEditTarget(spec);
    setEditForm({ name: spec.name, description: spec.description || '', department_id: spec.department_id, image: spec.image || '' });
    setEditErrors({});
  };
  ...
  const handleEditSubmit = async () => {
    ...
    if (!editForm.department_id) errors.department_id = 'Please select a department.';
    if (Object.keys(errors).length > 0) { setEditErrors(errors); return; }

    setIsEditSubmitting(true);
    await new Promise((res) => setTimeout(res, 900));
    setSpecializations((prev) =>
      prev.map((s) => s._id === editTarget?._id ? { ...s, ...editForm } : s)
    );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            ...
            <p className="text-slate-500 mt-1">Manage doctor specializations linked to departments.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-10">
          {[
            { label: 'Total Specializations', value: specializations.length, icon: Layers, color: 'text-slate-600', bg: 'bg-slate-100' },
            { label: 'Active', value: specializations.filter((s) => s.isActive).length, icon: CheckCircle2, color: 'text-emerald-600', bg: 'bg-emerald-100/50' },
            { label: 'Inactive', value: specializations.filter((s) => !s.isActive).length, icon: XCircle, color: 'text-rose-600', bg: 'bg-rose-100/50' },
            { label: 'Departments Covered', value: new Set(specializations.map((s) => s.department_id)).size, icon: Building2, color: 'text-blue-600', bg: 'bg-blue-100/50' },
          ].map((stat, i) => (

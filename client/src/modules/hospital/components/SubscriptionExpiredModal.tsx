...
const getDurationDisplay = (plan: ISubscription) => {
  ...
  if (plan.durationUnit && plan.durationUnit !== 'days') {
                ...
                subscriptionPlans.map((plan) => (
                  ...
                  >
                    ...
                    <div className="space-y-4 mb-8 flex-grow">
                      <PlanFeature label={`Up to ${plan.limits?.maxPatients || 'Unlimited'} Patients`} />
                      <PlanFeature label={`Up to ${plan.limits?.maxDoctors || 'Unlimited'} Doctors`} />
                      <PlanFeature label={`Up to ${plan.limits?.maxDepartments || 'Unlimited'} Depts`} />
                      {plan.features?.map((f, i) => (
                        <PlanFeature key={i} label={f} />
                      ))}
                    </div>
            ...
            {totalPages > 1 && (
              <div className="mt-12 flex justify-center items-center gap-4">
                <button
                  ...
                  className="px-4 py-2 bg-white border border-slate-200 rounded-xl font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
                  Previous
                </button>

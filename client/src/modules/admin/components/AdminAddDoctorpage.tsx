...
const AdminAddDoctorpage: React.FC = () => {
  ...
  const onSubmit = async (data: AdminAddDoctorpageFormData) => {
    try {
      ...
      navigate(ADMINROUTES.ADMINDOCTORMANGEMENT);
      reset();
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {['qualification', 'experience', 'department'].map((field) => (
            <div key={field}>
              <input
                {...register(field as keyof AdminAddDoctorpageFormData)}
                type="text"
                placeholder={field.charAt(0).toUpperCase() + field.slice(1)}
                className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors[field as keyof AdminAddDoctorpageFormData] ? 'border-red-500' : 'border-gray-300'
                  }`}
              />
              {errors[field as keyof AdminAddDoctorpageFormData] && (
                <p className="mt-1 text-sm text-red-500">
                  {errors[field as keyof AdminAddDoctorpageFormData]?.message}
                </p>
              )}
            </div>
          ))}
          ...
          <div>
            <input
              ...
              className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.specialization ? 'border-red-500' : 'border-gray-300'
                }`}
            />

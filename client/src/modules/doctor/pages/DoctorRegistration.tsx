          ...
          {['qualification', 'experience', 'department'].map((field) => (
            <div key={field}>
              <input
                ...
                className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors[field as keyof DoctorRegistrationFormData] ? 'border-red-500' : 'border-gray-300'
                  }`}
              />
              {errors[field as keyof DoctorRegistrationFormData] && (
                <p className="mt-1 text-sm text-red-500">
                  {errors[field as keyof DoctorRegistrationFormData]?.message}
                </p>
              )}
            </div>
          ))}

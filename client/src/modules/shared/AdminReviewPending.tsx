...
const AdminReviewPending: React.FC = () => {
  ...
  const handleEdit = async (hospital: IHospital) => {
    ...
    if (initializeAuth.fulfilled.match(resultAction)) {
      ...
      if (freshStatus === 'revision') {
        const destination = role === 'doctor' ? DOCTOR_ROUTES.DOCTOREDITFORREVIEW : HOSPITAL_ROUTES.HOSPITALEDITFORREVIEW;
  ...
  const handleReapply = async (id: string) => {
    try {
      if (userRole === "doctor") {
        await doctorApi.reapply(id);
      } else {
        await hospitalApi.reapply();
      }
      ...
      <div className="w-full max-w-2xl bg-white shadow-xl rounded-2xl p-8 border border-gray-100">
        <div className="text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
            {isRejected && (userData?.id || userData?._id) && (
              <button
                onClick={() => handleReapply((userData.id || userData._id) as string)}
            ...
            {isRevision && userData && (
              <button
                onClick={() => handleEdit(userData as unknown as IHospital)}
